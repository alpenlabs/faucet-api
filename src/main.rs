...
async fn main() {
    ...
    let app = Router::new()
        ...
        .layer(state.settings.ip_src.clone().into_extension())
        .with_state(state);
...
async fn claim_l1(
    ...
    let address = address
        .require_network(state.settings.network)
        .map_err(|_| {
            (
                StatusCode::BAD_REQUEST,
    ...
    let psbt = {
        ...
        if balance.trusted_spendable() < state.settings.sats_per_claim {
            return Err((
        ...
        let mut psbt = l1w
            ...
            .fee_rate(fee_rate())
            .enable_rbf()
            .add_recipient(address.script_pubkey(), state.settings.sats_per_claim)
            .clone()
            .finish()
